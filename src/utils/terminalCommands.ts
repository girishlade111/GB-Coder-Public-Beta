...
export class TerminalCommandProcessor {
  ...
  private showAbout(): TerminalOutput[] {
    ...
    return [{
      id: Date.now().toString(),
      type: 'success',
      message: '📄 Navigating to About Us page...',
      timestamp: new Date().toISOString(),
    }];
  }
...
│   toggle theme           Toggle between dark/light theme   │
│                                                             │
│ SNIPPET COMMANDS                                            │
...
╰─────────────────────────────────────────────────────────────╯`;
  ...
  private toggleTheme(args: string[]): TerminalOutput[] {
    if (args.length === 0 || args[0] !== 'theme') {
      return [{
        id: Date.now().toString(),
        type: 'error',
        message: '❌ Usage: toggle theme',
        timestamp: new Date().toISOString(),
      }];
    }
  ...
  private npmInstall(packages: string[]): TerminalOutput[] {
    ...
    localStorage.setItem('gb-coder-npm-packages', JSON.stringify(this.state.npmPackages));
  ...
  private showVersion(): TerminalOutput[] {
    return [{
      id: Date.now().toString(),
      type: 'info',
      message: `🚀 GB Coder Terminal v1.0.0\nBuilt with React + TypeScript\nPowered by Monaco Editor`,
      timestamp: new Date().toISOString(),
    }];
  }
